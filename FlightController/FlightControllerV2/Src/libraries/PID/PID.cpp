...
#if 0
    // Here it is assumed that timing control lies outside of this library. Everytime this API gets
    // called, exactly SampleTime has passed.
    unsigned long timeChange = dt;
...
void PID::SetTunings(float Kp, float Ki, float Kd, int POn)
{
    ...
    if(controllerDirection == PID_CTRL_DIR_REVERSE)
    {
        kp = (0 - kp);
        ki = (0 - ki);
        kd = (0 - kd);
