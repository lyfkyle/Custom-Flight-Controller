...
bool SBUS_Read(SBUSDataType* pSBUSData)
{
    ...
    channels_int[0]  = (uint16_t) ((receivedMsg[0]    | receivedMsg[1] <<8)                       & 0x07FF);
    channels_int[1]  = (uint16_t) ((receivedMsg[1]>>3 | receivedMsg[2] <<5)                       & 0x07FF);
    channels_int[2]  = (uint16_t) ((receivedMsg[2]>>6 | receivedMsg[3] <<2 | receivedMsg[4]<<10)  & 0x07FF);
    channels_int[3]  = (uint16_t) ((receivedMsg[4]>>1 | receivedMsg[5] <<7)                       & 0x07FF);
    channels_int[4]  = (uint16_t) ((receivedMsg[5]>>4 | receivedMsg[6] <<4)                       & 0x07FF);
    channels_int[5]  = (uint16_t) ((receivedMsg[6]>>7 | receivedMsg[7] <<1 | receivedMsg[8]<<9)   & 0x07FF);
    channels_int[6]  = (uint16_t) ((receivedMsg[8]>>2 | receivedMsg[9] <<6)                       & 0x07FF);
    channels_int[7]  = (uint16_t) ((receivedMsg[9]>>5 | receivedMsg[10]<<3)                       & 0x07FF);
    channels_int[8]  = (uint16_t) ((receivedMsg[11]   | receivedMsg[12]<<8)                       & 0x07FF);
    channels_int[9]  = (uint16_t) ((receivedMsg[12]>>3| receivedMsg[13]<<5)                       & 0x07FF);
    channels_int[10] = (uint16_t) ((receivedMsg[13]>>6| receivedMsg[14]<<2 | receivedMsg[15]<<10) & 0x07FF);
    channels_int[11] = (uint16_t) ((receivedMsg[15]>>1| receivedMsg[16]<<7)                       & 0x07FF);
    channels_int[12] = (uint16_t) ((receivedMsg[16]>>4| receivedMsg[17]<<4)                       & 0x07FF);
    channels_int[13] = (uint16_t) ((receivedMsg[17]>>7| receivedMsg[18]<<1 | receivedMsg[19]<<9)  & 0x07FF);
    channels_int[14] = (uint16_t) ((receivedMsg[19]>>2| receivedMsg[20]<<6)                       & 0x07FF);
    channels_int[15] = (uint16_t) ((receivedMsg[20]>>5| receivedMsg[21]<<3)                       & 0x07FF);
    // parse
    for (int i = 0; i < 16; ++i) {
        pSBUSData->channels[i] = ((float) (channels_int[i] - SBUS_CHANNEL_MIN)) / (SBUS_CHANNEL_MAX - SBUS_CHANNEL_MIN) * (sChannelOutMin - sChannelOutMin) + sChannelOutMin;
    }
