...
ringbuf_t* ringbuf_new(uint32_t capacity)
{
...
uint32_t ringbuf_buffer_size(const ringbuf_t *rb)
{
    return rb->size;
}
...
uint32_t ringbuf_bytes_free(const ringbuf_t *rb)
{
    if (rb->head >= rb->tail)
...
uint32_t ringbuf_bytes_used(const ringbuf_t *rb)
{
...
const uint8_t* ringbuf_tail(const ringbuf_t *rb)
{
    return rb->tail;
}

const uint8_t* ringbuf_head(const ringbuf_t *rb)
{
    return rb->head;
}
...
void* ringbuf_memcpy_into(ringbuf_t* dst, const void *src, uint32_t count)
{
...
void* ringbuf_memcpy_from(void *dst, ringbuf_t* src, uint32_t count)
{
    uint32_t bytes_used = ringbuf_bytes_used(src);
    if (count > bytes_used)
        return 0;
...
#if 0
uint32_t
ringbuf_findchr(const struct ringbuf_t *rb, int c, uint32_t offset)
{
    const uint8_t *bufend = ringbuf_end(rb);
    uint32_t bytes_used = ringbuf_bytes_used(rb);
    ...
    assert(bufend > start);
    uint32_t n = MIN(bufend - start, bytes_used - offset);
...
uint32_t
ringbuf_memset(ringbuf_t dst, int c, uint32_t len)
{
    const uint8_t *bufend = ringbuf_end(dst);
    uint32_t nwritten = 0;
    uint32_t count = MIN(len, ringbuf_buffer_size(dst));
    int overflow = count > ringbuf_bytes_free(dst);
    ...
    while (nwritten != count) {
        ...
        uint32_t n = MIN(bufend - dst->head, count - nwritten);
        memset(dst->head, c, n);
        dst->head += n;
        nwritten += n;
...
uint32_t
ringbuf_read(int fd, ringbuf_t rb, uint32_t count)
{
    const uint8_t *bufend = ringbuf_end(rb);
    uint32_t nfree = ringbuf_bytes_free(rb);

    /* don't write beyond the end of the buffer */
    assert(bufend > rb->head);
    count = MIN(bufend - rb->head, count);
    suint32_t n = read(fd, rb->head, count);
    if (n > 0) {
...
uint32_t ringbuf_write(int fd, ringbuf_t rb, uint32_t count)
{
    uint32_t bytes_used = ringbuf_bytes_used(rb);
    if (count > bytes_used)
        return 0;
    ...
    suint32_t n = write(fd, rb->tail, count);
    if (n > 0) {
...
void* ringbuf_copy(ringbuf_t dst, ringbuf_t src, uint32_t count)
{
    uint32_t src_bytes_used = ringbuf_bytes_used(src);
    ...
    while (ncopied != count) {
        ...
        uint32_t nsrc = MIN(src_bufend - src->tail, count - ncopied);
        assert(dst_bufend > dst->head);
        uint32_t n = MIN(dst_bufend - dst->head, nsrc);
